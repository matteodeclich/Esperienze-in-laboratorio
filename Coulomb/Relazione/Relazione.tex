...
Una volta misurata la costante di torsione del filo, sfruttando la legge del momento torcente, sarà possibile ottenere la costante dielettrica $\varepsilon_{0}$ del vuoto e confrontarla col valore atteso di \qty{8.85e-12}{\coulomb\per\newton\per\meter\tothe{2}}.
...
Come mostrato in Figura 1, una sfera leggera, ricoperta di vernice conduttiva, è sostenuta da un braccio rigidamente collegato ad un equipaggio mobile, contrappesato e ancorato a sua volta a un sottile filo di torsione. La sfera può ruotare torcendo il filo. Una sfera identica è montata su un supporto a slitta, in modo che si possa posizionare a diverse distanze rispetto la prima.
...
\begin{equation}
    \tau_{\text{Coulomb}} = F_C \cdot b = C_{\text{tor}} \cdot \theta = \tau_{tor}
    \label{eq:Momento}
\end{equation}
...
Il coefficiente angolare della retta interpolata è $k_1=\qty{17.5 \pm 0.1 e-3}{\meter\tothe{2}}$. Il test del $\chi^2$ restituisce $\tilde{\chi}_o^2= 0.44$, che per 4 gradi di libertà corrisponde a una probabilità di accordo con i dati del $78\%$.

\section{Dipendenza di \boldmath $F_C$ dalla carica}
\subsection{Potenziale Uguale}
Per determinare la dipendenza della forza di Coulomb dalla carica è necessario eseguire misure nelle stesse modalità del punto precedente, mantenendo però fissa la distanza tra le due sfere. Le misure sono state effettuate ad una distanza di $(12.0 \pm 0.1)$ cm. 
A piccole distanze la capacità delle sfere è circa costante. Poiché la carica è direttamente proporzionale al potenziale, secondo la relazione $q = C\cdot V$, dove $C=4\pi \varepsilon_{0} a$ è la capacità di una sfera isolata, è sufficiente studiare, tramite le relazioni \eqref{Coulomb} e \eqref{eq:Momento}, la dipendenza della forza dal potenziale.
\newpage

Si caricano le sfere allo stesso potenziale, con valori crescenti fino a 6 kV. Per il potenziale si è stimato un errore dell'1\%, tenendo in considerazione inevitabili dispersioni di carica. Per gli angoli, l'incertezza di 4° è dovuta ad una combinazione di possibili errori sistematici di azzeramento della strumentazione, di sensibilità ed oscillazioni residue. Le misure sono state raccolte in Tabella 2.
...
\midrule
2000 & 20 & 10 & 0.17 & 0.07 \\
2500 & 25 & 14 & 0.24 & 0.07 \\
3000 & 30 & 20 & 0.35 & 0.07 \\
3500 & 35 & 23 & 0.40 & 0.07 \\
4000 & 40 & 35 & 0.61 & 0.07 \\
4500 & 45 & 41 & 0.72 & 0.07 \\
5000 & 50 & 48 & 0.84 & 0.07 \\
5500 & 55 & 62 & 1.08 & 0.07 \\
...
Si interpolano linearmente col metodo dei minimi quadrati gli angoli di torsione in funzione del potenziale $V^2$, applicando agli angoli la correzione 1/B, come in Figura 3.
...
Il coefficiente angolare della retta interpolata è $k_2=\qty{35.5 \pm 1.2}{\nano\volt\tothe{2}}$. Il test di adattamento del $\chi^2$ restituisce un $\tilde{\chi_o}^2= 0.25$, che per 8 gradi di libertà corrisponde ad una probabilità di accordo con i dati del $98\%$.
\newpage

\subsection{Potenziale Diverso}
Si ripetono le misure mantenendo costante il potenziale su una sfera e variandolo sull'altra. È stato considerato un potenziale fisso pari a 6 kV, mentre gli errori sono stati stimati come nel punto precedente. In Tabella 3 sono riportati i valori misurati.
...
{\text{(V)}}  & {\text{(V)}}  & {\text{(°)}} & {\text{(rad)}} & {\text{(rad)}} \\
\midrule
2000 & 20 & 28 & 0.49 & 0.07 \\
2500 & 25 & 35 & 0.61 & 0.07 \\
3000 & 30 & 39 & 0.68 & 0.07 \\
3000 & 30 & 39 & 0.68 & 0.07 \\
3500 & 35 & 43 & 0.75 & 0.07 \\
4000 & 40 & 52 & 0.91 & 0.07 \\
4500 & 45 & 59 & 1.03 & 0.07 \\
5000 & 50 & 61 & 1.06 & 0.07 \\
5500 & 55 & 65 & 1.13 & 0.07 \\
6000 & 60 & 70 & 1.22 & 0.07 \\
\bottomrule
\end{tabular}
...
Si procede ad interpolare gli angoli di torsione in funzione del potenziale variabile, applicando la correzione 1/B, assumendo un parametro libero come mostrato in Figura 4.
...
All'equilibrio si eguagliano i momenti della forza di gravità $F_G$ e di quella di torsione, e si ha $F_G \cdot b = m\cdot g \cdot b = C_{\text{tor}} \cdot \theta$,
...
Dunque, a partire dai coefficienti angolari precedentemente interpolati, si calcolano i valori di $\varepsilon_0$ e se ne fa una media pesata da cui si ottiene $\bar{\varepsilon}_{0} = \qty{8.36 \pm 0.52 e-12}{\coulomb\per\newton\per\meter\tothe{2}}$
...
La media pesata $\bar{\varepsilon}_{0}$ delle tre misure risulta compatibile col valore previsto $\varepsilon_0 = \qty{8.85 e-12}{\coulomb\per\newton\per\meter\tothe{2}}$ con una probabilità del $34\%$, corrispondente a:
