...
Inoltre, la legge di Snell prevede la reversibilità del cammino ottico. Se faccio incidere il raggio luminoso ad un angolo pari a $\theta_{\mathrm{\text{rifratto}}}$ devo ottenere \(\theta'_{\mathrm{\text{rifratto}}}=\theta_{\mathrm{incidenza}}\).

Questa prima parte dell'esperimento è stata effettuata utilizzando una lente semicilindrica in plexiglass. Facendo incidere il raggio luminoso nel centro della lente si ha una sola rifrazione nel passaggio dall'aria al plexiglass, poiché il raggio incide la superficie con un angolo nullo rispetto alla tangente alla curva nel passaggio dalla lente all'aria. 
...
\newpage
Si riportano qui i dati raccolti in una tabella:
...
È stata fatta quindi un'interpolazione lineare passante per l'origine tra $\sin\theta_{\text{inc}}$ e $\sin\theta_{\text{rif}}$ dalla quale si ricava k=0.66 $\pm$ 0.01. Il test di adattamento del $\chi^2$ restituisce un p-value del $99.3\%$, mostrando un'ottima compatibilità con il modello lineare.
...
Approssimando $n_{\text{aria}}=1$, si ottiene\\ $n_{\text{plexiglass}}=1/k=1.50 \pm 0.02$.
\newpage
Le stesse misure sono state ripetute facendo incidere il raggio luminoso sulla parte curva della lente.
Per entrambi i lati della lente è stata quindi verificata la reversibilità del cammino ottico. Si riportano di seguito i dati:
...
25$^\circ$ & 16$^\circ$ & 16$^\circ$ & 24$^\circ$ & & 25$^\circ$ & 39$^\circ$  & 25$^\circ$  & 25$^\circ$ \\
...
60$^\circ$ & 35$^\circ$ & 35$^\circ$ & 57$^\circ$ & & 60$^\circ$ & 120$^\circ$ & 121$^\circ$ & 58$^\circ$ \\
70$^\circ$ & 39$^\circ$ & 39$^\circ$ & 68$^\circ$ & & 70$^\circ$ & 111$^\circ$ & 111$^\circ$ & 69$^\circ$ \\
\bottomrule
\end{tabular}
\caption{Confronto diretto degli angoli incidenti e rifratti per la reversibilità. Sono state omesse le incertezze di \ang{1} su tutte le misure}
\label{tab:confronto_diretto}
\end{table}
Le misure indicano in generale una compatibilità tra $\theta_{\text{\text{inc}}}$ e $\theta'_{\text{\text{rif}}}$ entro due deviazioni standard. L'unica misura che presenta una compatibilità dubbia è per $\theta'_{\text{\text{rif}}}$=$\ang{57}$, nelle misure per il lato piatto della lente. Il t-test restituisce infatti t=2.12, che corrisponde a una compatibilità del 4.4\%, di poco sotto alla soglia di accettabilità standard del 5\%.\\
Infine, è stato misurato l'angolo critico sul lato curvo, ovvero l'angolo di incidenza che realizza la riflessione totale, ovvero quando quando il raggio luminoso viene rifratto parallelamente al lato piatto della lente e svanisce.\\ Il fenomeno si spiega con la legge di Snell: quando si ha riflessione totale $\theta_{\text{rif}}=\ang{90}$, dunque $\sin\theta_{\text{rif}}=1$, e per valori di $\theta_{inc}$ maggiori di $\theta_{critico}$ si dovrebbe avere $$\sin\theta_{rif}=\frac{n_1}{n_2}\sin\theta_{inc}>1$$ il che è impossibile. Quindi per angoli maggiori di $\theta_{critico}$  non si ha più rifrazione, ma riflessione.
Per la lente semicilindrica il valore ottenuto è $\theta_{\text{critico}}=\qty{44 \pm 1}{\degree}$, da cui si può ricavare una seconda stima indipendente dell'indice di rifrazione del prlexiglass. Sempre approssimando $n_{aria}=1$, si ricava infatti $$n_{\text{plexglass}} = \frac{1}{\sin\theta_{\text{critico}}} = 1.49 \pm 0.03$$
Il test di compatibilità tra questa e quella precedente restituisce un valore di $t=0.17$, che corrisponde a 86.5\% di compatibilità tra le due misure. \\
La riflessione totale spiega inoltre anche il fatto che non ci sia una relazione lineare tra i seni calcolati per le misure sul lato curvo della lente: le ultime 3 misure sono state effettuate oltre al limite dell'angolo critico, ottenendo così una riflessione invece che una rifrazione. 