...
Le misure dell'angolo di deviazione minima sono state effettuate sia per un prisma triangolare equilatero sia per un prisma trapezoidale in corrispondenza dello spigolo $\alpha=45^\circ$.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Prisma} & $\bm{\delta_{\text{min}}}$ \textbf{\small{(\unit{rad})}} & $\bm{\alpha}$ \textbf{\small{(\unit{rad})}} & $\mathbf{n}$ & $\bm{\sigma_{\text{n}}}$ \\
\midrule
